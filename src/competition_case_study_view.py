...
.tg-case-card {border:1px solid #dce4ef;border-radius:18px;padding:.92rem;background:#fff;min-height:340px;box-shadow:0 8px 22px rgba(15,36,68,.05);}
...
def build_official_case_study_summaries() -> list[dict[str, Any]]:
    """Return UI-ready summaries without performing a network call."""
    ...
    for case in dataset.cases:
        ...
        trade_key = (
            "un_comtrade_export" if case.trade_flow_code == "X" else "un_comtrade_import"
        )
        trade_source = by_key[trade_key]

        macro_payload = macro_source.payload if isinstance(macro_source.payload, dict) else {}
        macro_rows = macro_payload.get("results") or []
        metrics = []
        for wrapper in macro_rows:
        ...
        summaries.append(
            {
                ...
                "macro_retrieved_at": (
                    macro_source.retrieved_at.isoformat() if macro_source.retrieved_at else None
                ),
                "trade_retrieved_at": (
                    trade_source.retrieved_at.isoformat() if trade_source.retrieved_at else None
                ),
...
def render_official_case_study_section(*, presentation_mode: bool) -> None:
    """Render pinned public context separately from optional live API controls."""
    ...
    st.markdown(
        '<div class="tg-section-title">06 · 실제 공개데이터 사례 3개</div>',
    ...
    for column, item in zip(columns, summaries):
        with column:
            ...
            trade_value = (
                _format_usd(item["trade_value_usd"])
                if item["trade_value_usd"] is not None
                else "자료 없음"
            )
            st.markdown(
                f'''
